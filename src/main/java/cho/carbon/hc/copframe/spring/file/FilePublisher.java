...
public class FilePublisher {
	...
	public InputStream getFileBodyIS(String fileCode) {
		...
		if (bytes == null) {
			publishBytes(fileHauntMap.get(fileCode));
			bytes = fileBytesMap.get(fileCode);
		}
		if (bytes != null) {
	...
	public String publish(FileHaunt file) {
		if (file.getCode() != null && fileNameMap.containsKey(file.getCode())) {
			return getURL(file);
		} else {
			return publish(file, false);
		}
	}

	private String publish(FileHaunt file, boolean includeBytes) {
	...
	private String publishBytes(FileHaunt file) {
		if (file.getCode() != null && fileBytesMap.containsKey(file.getCode())) {
			return getURL(file);
		} else {
			return publish(file, true);
		}
	}
